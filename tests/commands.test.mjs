...
test("rescue command absorbs continue semantics", () => {
  ...
  assert.match(rescue, /Run Codex directly through the companion helper/i);
  assert.match(rescue, /^context:\s*fork\b/m);
  assert.match(rescue, /allowed-tools:\s*mcp__shell__run,\s*Bash\(node:\*\)/);
  assert.match(rescue, /Do not invoke the Skill tool, Task tool, Agent tool/i);
  assert.match(rescue, /Make exactly one `mcp__shell__run` call/i);
  assert.match(rescue, /Use `Bash\(node:\*\)` only as a fallback/i);
  assert.match(rescue, /--background\|--wait/);
  assert.match(rescue, /--no-monitor/);
  assert.match(rescue, /--resume\|--fresh/);
  assert.match(rescue, /--model <model\|spark>/);
  assert.match(rescue, /--effort <none\|minimal\|low\|medium\|high\|xhigh>/);
  assert.match(rescue, /Convert `--resume` to `--resume-last`/);
  assert.match(rescue, /If neither resume nor fresh is present, add `--fresh`/);
  assert.match(rescue, /Add `--write` unless the user explicitly asks for read-only behavior/);
  assert.match(rescue, /Map `--model spark` to `--model gpt-5\.3-codex-spark`/);
  assert.match(rescue, /Return the companion stdout verbatim to the user/i);
  assert.doesNotMatch(rescue, /AskUserQuestion/);
  assert.doesNotMatch(rescue, /subagent_type/);
  ...
  assert.match(agent, /single `mcp__shell__run` call/i);
  assert.match(agent, /A response with zero tool uses is invalid/i);
  ...
  assert.match(agent, /task helper opens a visible Alacritty monitor terminal by default/i);
  assert.match(agent, /Only pass `--no-monitor` when the user explicitly requested no visible monitor/i);
  ...
  assert.match(runtimeSkill, /Preferred executor:\n- Use `mcp__shell__run` with the primary helper command/i);
  ...
  assert.match(runtimeSkill, /helper opens a visible Alacritty monitor terminal for task handoffs by default/i);
  assert.match(runtimeSkill, /If the forwarded request includes `--no-monitor`, pass it through to `task`/i);
  ...
  assert.match(readme, /companion runtime and opens a visible sidecar monitor by default/i);
